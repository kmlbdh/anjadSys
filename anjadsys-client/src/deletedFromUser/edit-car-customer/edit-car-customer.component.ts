...
import { combineLatest, debounceTime, distinctUntilChanged, Observable, Subject, switchMap, takeUntil } from 'rxjs';
import { CarAPI, CarModelAPI, CarsAPI, CarTypeAPI, CarTypeArrayAPI, updateCar } from 'src/app/modules/core/model/car';
...
export class EditCarCustomerComponent implements OnInit, OnDestroy {
  ...
  updateCar = (): void => {
    ...
    this.agentService.updateCar(formObj)
  ...
  searchCarModelAPI(triggerBuildForm: boolean = false){
    ...
    this.agentService.listCarModels({carTypeId: Number(carTypeId), skipLoadingInterceptor: true})
    .pipe(takeUntil(this.unsubscribe$))
    .subscribe({
      next: response => {
  ...
  searchCustomerAPI(){
    this.searchCustomerText$.pipe(
      ...
      switchMap(text => this.agentService.showUsers({username: text, skipLoadingInterceptor: true}))
    ).subscribe({
      next: (response: any) =>{
        if(response.data){
  ...
  getCarData(): void{
    this.route.paramMap.subscribe({
      next: params => {
        ...
        let car$ = this.agentService.showCars({carId:  Number(carId!)}) as Observable<CarsAPI>;
        let carType$ = this.agentService.listCarTypes({}) as Observable<CarTypeArrayAPI>;
  ...
  redirect(): void{
    this.router.navigate(['/agent/car/car-customer/show']);
  }
