...
import { combineLatest, debounceTime, distinctUntilChanged, filter, forkJoin, mergeMap, Observable, of, Subject, switchMap, takeUntil, tap } from 'rxjs';
import { CarAPI, SearchCar } from 'src/app/modules/core/model/car';
import { InsurancePolicesAPI, InsurancePolicyAPI, updateInsurancePolicy } from 'src/app/modules/core/model/insurancepolicy';
import { ServiceAPI, ServicePolicyAPI, ServicesAPI, updateServicePolicy } from 'src/app/modules/core/model/service';
...
export class EditInsurancePolicyComponent implements OnInit {
    ...
    this.route.paramMap.subscribe({
      next: params => {
        ...
        if(!insurancePolicyID)
          this.router.navigate(['/agent/insurance-policy/show']);
  ...
  getData(insurancePolicyID: string){
    ...
    let insurancePolicy$ = this.agentService.listInsurancePolicy({insurancePolicyId: Number(insurancePolicyID)}) as Observable<InsurancePolicesAPI>;
    let services$ = this.agentService.listServices() as Observable<ServicesAPI>;
  ...
  updateInsurancePolicy = (ngform: FormGroupDirective) => {
    ...
    this.agentService.updateInsurancePolicy(formObj)
      .pipe(takeUntil(this.unsubscribe$))
      .subscribe({
        next: (response) => {
          if(response.data){
  ...
  searchCustomerAPI(){
    let callback = (val: string) => this.agentService.showUsers(
