...
import { NewServicePolicy, ServiceAPI } from '@models/service';
...
export class AddInsurancePolicyComponent implements OnInit, OnDestroy {
  ...
  totalCoverageDays(serviceId: number): number {
    let serviceDefualtDays = Number(this.services.filter(service =>
  ...
  totalCostForAllServices() {
    const total = this.servicesPolicy.reduce((sum, service) => sum += service.cost, 0);
